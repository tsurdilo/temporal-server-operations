...
| `temporal_request_failure` | `StartWorkflowExecution` | `Unavailable` / `Internal` | Both are retried automatically by the SDK. A single occurrence or intermittent low-rate occurrences can be transient server issues — no immediate alert needed if the SDK retries succeed. However even when retries ultimately succeed, each retry introduces backoff delay before the workflow actually starts. In high throughput use cases this can have direct business impact — workflows that should start immediately are delayed by retry backoff cycles, and the cumulative effect under sustained intermittent errors can be significant. If seen continuously or at a high rate approaching the SDK retry window (~60s), alert immediately as executions will start being dropped. The severity depends heavily on your use case — a low-throughput batch process tolerates retry delays far better than a real-time request handling system. Ensure your client code handles the final failure after SDK retries are exhausted by logging the workflow ID and input payload so lost executions can be backfilled. |
...
| `temporal_request_failure` | `RespondWorkflowTaskFailed` | `NotFound` (elevated rate) | Same causes and same urgency as `RespondWorkflowTaskCompleted` `NotFound` — WFT timed out, execution completed, or run/execution timeout fired. Alert immediately if elevated and not explainable by known terminations or timeouts. |
| `temporal_request_failure` | `RespondWorkflowTaskFailed` | `ResourceExhausted` | Workers cannot report WFT failures back to the server. Lower priority than completion (P3) — under extreme load these may be dropped, causing WFTs to time out instead. Same feedback loop risk as `RespondWorkflowTaskCompleted` `ResourceExhausted`. Alert if seen at high rate. |
| `temporal_request_failure` | `RespondActivityTaskFailed` | `ResourceExhausted` | Workers cannot report activity failures back to the server. Lower priority than completion (P3) — under extreme load these may be dropped, causing activities to time out instead of failing cleanly. Alert if seen at high rate. |
...
| `temporal_request_failure` | `RespondActivityTaskCompleted` | `NotFound` (elevated rate) | Activities are timing out before workers can respond. Indicates `StartToCloseTimeout` is too tight for the actual activity duration, workers are struggling under load, high worker resource utilization, or high server-side persistence latencies. Direct business impact as activities will retry or fail. |
| `temporal_request_failure` | `ShutdownWorker` | `ResourceExhausted` / `Unavailable` / `Internal` | Alert immediately. `ShutdownWorker` is assigned Priority 4 by the server but is operationally far more critical than that suggests. It tells the server to unload the worker's sticky task queue and move outstanding workflow tasks back to the normal task queue so other workers can pick them up. If this call fails for any reason, those tasks remain pinned to a shutting-down worker, causing execution delays and workflow task timeouts until the sticky queue TTL expires. This is especially impactful during rolling deployments where workers are being replaced frequently. |
...
| `temporal_request_failure` | `UpdateWorkflowExecution` | `DeadlineExceeded` (sustained) | Synchronous update calls are timing out — same signal as `PollWorkflowExecutionUpdate` `DeadlineExceeded`. |
...
| `temporal_request_failure` | `SignalWorkflowExecution` | `NotFound` (elevated rate) | Workflow not found — it completed, was terminated, or exceeded retention. If seen at an elevated rate this is worth investigating — it may indicate workflows are completing or timing out faster than expected, or that signal senders are not checking workflow state before signaling. |
| `temporal_request_failure` | `StartWorkflowExecution` | `AlreadyExists` (elevated rate, unexpected) | A workflow with this ID is already running. If this is expected behavior (intentional deduplication), this is low impact. If unexpected, an elevated rate indicates duplicate start calls or ID generation issues that need investigation — workflows that should have started are being silently dropped. |
...
| `StartWorkflowExecution` | `Unavailable` | Retried automatically by the SDK, but each retry introduces backoff delay before the workflow actually starts. In high throughput use cases this can have direct business impact even for short durations — workflows are delayed rather than dropped, but the cumulative effect of retry backoffs under sustained intermittent errors can be significant. If seen continuously or approaching the SDK retry window (~60s), alert. Severity depends heavily on use case — a real-time request handling system is far more sensitive to this than a batch process. Log workflow ID and input payload on final failure. |
| `StartWorkflowExecution` | `Internal` | Same as `Unavailable` — retried automatically with backoff. Same retry delay impact applies under high throughput. Log workflow ID and input payload on final failure. |
...
| `RecordActivityTaskHeartbeat` | `ResourceExhausted` | Alert on this. Can be caused by RPS throttling — check `frontend.namespaceRPS` and the Resource Exhausted with Cause panel on your server dashboard. Can also be caused by the heartbeat details payload exceeding the 4 MB gRPC message size limit. In either case this is serious — the SDK will retry the heartbeat with backoff, but if the throttling persists long enough that the cumulative retry delays cause the `HeartbeatTimeout` to fire before a successful heartbeat gets through, the server will mark the activity as timed out and reschedule it. If the activity code properly handles the cancellation signal returned via the heartbeat response it will stop gracefully. If it ignores cancellation and runs to completion anyway, this will also surface as `NotFound` on `RespondActivityTaskCompleted`. Note however that seeing `ResourceExhausted` on heartbeats and `NotFound` on completions at the same time does not necessarily mean one caused the other — the `NotFound` could be independently caused by high worker CPU/memory utilization slowing down activity processing, a batch of workflow executions being terminated with pending activities, or many execution/run timeouts firing with activities that were still running and completed after the fact. Always investigate each signal on its own merits before assuming a causal relationship. |
...
| `DescribeNamespace` | `ResourceExhausted` | Server-side throttling at Priority 0 — extremely rare since P0 is shed last. Alert on this — `DescribeNamespace` is called by workers and clients at startup to validate and load namespace configuration. Failures here can cause worker startup delays or failures, preventing workers from coming online. Check the Resource Exhausted with Cause panel on your server dashboard. |
| `DescribeNamespace` | `Unavailable` | Typically related to server issues — frontend unreachable or other server-side errors. Alert on this even if intermittent — failures here can cause worker startup delays or failures. Check server health immediately. |
| `DescribeNamespace` | `Internal` | Typically unexpected server-side issues. Alert on this — failures during worker startup have direct business impact. Check server health. |
| `RegisterNamespace` | `AlreadyExists` | Namespace already registered. |
| `RegisterNamespace` | `ResourceExhausted` | NamespaceReplicationInducing bucket exhausted — default global limit is 20 RPS cluster-wide shared across all namespaces. Check the Resource Exhausted with Cause panel on your server dashboard. |
| `RegisterNamespace` | `InvalidArgument` | Bad namespace name, invalid retention period, or unsupported config. |
| `RegisterNamespace` | `Unavailable` | Intermittent server issues — frontend unreachable or other server-side errors. Can be intermittent during server scaling or restarts. If seen persistently, alert and check server health. |
| `RegisterNamespace` | `Internal` | Intermittent server issues. If seen at a higher rate or over a longer duration, alert and check server health. |
| `UpdateNamespace` | `NotFound` | Namespace not found. |
| `UpdateNamespace` | `ResourceExhausted` | NamespaceReplicationInducing bucket exhausted — 20 RPS global default shared across all namespaces. Check the Resource Exhausted with Cause panel on your server dashboard. |
| `UpdateNamespace` | `InvalidArgument` | Bad config — invalid retention, unsupported archival state, etc. Intermittent server issues can also surface here. |
...
| `GetClusterInfo` | `ResourceExhausted` | Server-side throttling at Priority 0 — extremely rare since P0 is shed last. Alert on this — `GetClusterInfo` is called by workers and clients at startup to discover cluster metadata. Failures here can cause worker startup delays or failures, preventing workers from coming online. Check the Resource Exhausted with Cause panel on your server dashboard. |
| `GetClusterInfo` | `Unavailable` | Typically related to server issues — frontend unreachable or other server-side errors. Alert on this even if intermittent — failures here can cause worker startup delays or failures. Check server health immediately. |
| `GetClusterInfo` | `Internal` | Typically unexpected server-side issues. Alert on this — failures during worker startup have direct business impact. Check server health. |
| `ShutdownWorker` | `ResourceExhausted` | Alert on this. Server-side throttling at Priority 4 — notably low given how operationally critical this call is. `ShutdownWorker` tells the server to unload the worker's sticky task queue and move any outstanding workflow tasks from the sticky queue back to the normal (non-sticky) task queue so other workers can pick them up. If this call is throttled and cannot go through, those workflow tasks remain pinned to a worker that is shutting down, causing delays in execution and potentially workflow task timeouts until the sticky queue TTL expires and the server reassigns them naturally. Check the Resource Exhausted with Cause panel on your server dashboard. |
| `ShutdownWorker` | `Unavailable` | Alert on this. Even intermittent failures here are worth alerting on — same consequences as `ResourceExhausted`: if the shutdown signal does not reach the server, sticky task queue cleanup is delayed, leading to execution delays and potential workflow task timeouts during worker deployments or restarts. Check server health. |
| `ShutdownWorker` | `Internal` | Alert on this. Same reasoning as `Unavailable` — failure to complete the shutdown signal has direct impact on sticky task queue cleanup and workflow task continuity. Check server health. |
...
| Dashboard Section | Panel | What to look for |
|-------------------|-------|-----------------|
| Server Overview — Service Requests and Errors | **Key API Latencies** (`service_latency_bucket{service_name="frontend"}` by `operation`) | **Check this first.** Look for elevated latency on the specific operation you are seeing `DeadlineExceeded` on. The dashboard tracks key operations including `SignalWorkflowExecution`, `StartWorkflowExecution`, `RespondWorkflowTaskCompleted`, and `RespondActivityTaskCompleted`. If latency is elevated here, the server is taking too long to process the request — then dig into persistence to find the root cause. |
...
| Server Overview — Service Requests and Errors | **Service Errors** (`service_errors` on frontend) | Check this alongside Key API Latencies — a spike here confirms the server is returning errors on that operation. Look at **Server Errors by Type** (`service_error_with_type` by `error_type`) to narrow down whether it is a DB issue, overload, or something else before drilling deeper. |
| Server Overview — Cluster Throughput | **Shard Lock Latency** (`semaphore_latency_bucket{operation="ShardInfo", service_name="history"}`) | If service errors are elevated, check this next. Elevated shard lock latency means history service shards are contended, which introduces latency to all operations and can cause timeouts. Orange threshold at 200ms, red at 400ms. |
