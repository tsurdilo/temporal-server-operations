...
>
> v1.31.1 handles this better than v1.30 — it does the work in one rebuild instead of several, so it's faster. But it's still a rebuild: on a very large table it can still take hours no matter which version you use, so plan a maintenance window either way. (See temporalio/temporal PR #10371.)
>
