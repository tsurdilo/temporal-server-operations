...
- **No burst allowance in this throttle.** This setting enforces a strict minimum spacing — it does not permit a short burst of starts closer together than the interval, so even a brief, legitimate burst of new runs on the same workflow ID is rejected. From **1.32.0**, `history.businessIDReuseRate` offers a rate limit that does allow a burst above the base rate, and it is checked before history is written rather than after (see the side effect below); covering it as a guardrail here is a planned update.
...
- **Side effect: leftover history (normally self-cleaning).** Each start this throttle rejects has already written its history a moment before the check runs, so that history is left with no workflow to own it. Temporal's **history scavenger** removes this on its own — it is normal, expected cleanup, not a permanent leak. The only thing to watch is volume: at a high rejection rate the history can build up faster than the scavenger's default schedule clears it, so keep the scavenger's wait (`worker.historyScannerDataMinAge`) low. See [History Growth from Duplicate Workflow Starts](./history-growth-duplicate-workflow-starts.md) for how to confirm it is keeping up. The burst-rate option noted above (`history.businessIDReuseRate`, from 1.32.0) is applied *before* history is written rather than after, so a start it turns away leaves nothing behind. How much that reduces the leftover history depends on the workload, though — for rapid restarts of one id it can help very little, because turned-away callers retry and each retry that slips through still writes history. That playbook has the measured comparison.
