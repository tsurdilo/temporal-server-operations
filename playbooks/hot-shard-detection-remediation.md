...
- **No burst allowance in this throttle.** This setting enforces a strict minimum spacing — it does not permit a short burst of starts closer together than the interval, so even a brief, legitimate burst of new runs on the same workflow ID is rejected. A newer setting, `history.businessIDReuseRate` (added in 1.32.0), is intended to allow a burst above the base rate, but per the Temporal server team its implementation is not finalized in 1.32 — don't rely on it yet.
...
- **Side effect: leftover history (normally self-cleaning).** Each start this throttle rejects has already written its history a moment before the check runs, so that history is left with no workflow to own it. Temporal's **history scavenger** removes this on its own — it is normal, expected cleanup, not a permanent leak. The only thing to watch is volume: at a high rejection rate the history can build up faster than the scavenger's default schedule clears it, so keep the scavenger's wait (`worker.historyScannerDataMinAge`) low. See [History Growth from Duplicate Workflow Starts](./history-growth-duplicate-workflow-starts.md) for how to confirm it is keeping up. (The newer `history.businessIDReuseRate`, from 1.32.0, is meant to run this check *before* the history write, but it is not finalized in 1.32 — don't rely on it yet.)
