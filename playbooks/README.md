...
| [Dual Visibility Operations](./dual-visibility.md) | Operating a cluster that writes every visibility record to two stores at once — the supported way to move between visibility stores without downtime, and a manual failover path. Not redundancy: nothing fails over on its own, and a read never falls back to the other store. Covers working out which store is failing (the signals differ by store type *and* by how it failed), the roughly one-hour window before dropped visibility records are lost for good, recovering from primary / secondary / both-stores-down outages, and moving reads and writes for a failover or a staged migration. Both stores on SQL (PostgreSQL / MySQL) or both on Elasticsearch. | [Temporal Server](../observability/dashboards/server/temporal-server-readme.md#16-visibility) v2.15.0+ | 059a, 059b, 059c, 083, 084, 085 |
