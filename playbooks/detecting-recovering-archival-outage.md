...
**Key point:** Temporal never *drops* a failing task — it retries it and, after enough failures, dead-letters it. So no DLQ setting can make the failing archival tasks disappear; the only fix is to **pause archival processing** while the backend is down. This playbook detects the condition early (well before the meltdown) and pauses.
