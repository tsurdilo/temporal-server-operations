...
- **Readme: `--dlq-type` is version-dependent.** `tdbg dlq read --dlq-type visibility` fails on v1.31.0 with `strconv.Atoi: parsing "visibility": invalid syntax`; that release wants the numeric category id (**4** for visibility). Newer builds accept the name. Also noted: without `--last-message-id` the command prompts, and with no terminal attached that prompt panics on EOF.

### Verified working against live data
Recorded here because these are the panels most likely to be doubted, and they behaved exactly as documented:
- **Visibility Read Request Rate per Store** tracked a read-routing change live — secondary 7 reads / primary 1 immediately after setting `system.enableReadFromSecondaryVisibility` for one namespace, with no restart.
- **Visibility Errors by Type per Store** attributed a hung store per index (36 / 36 across the two), while the bulk processor panels could not (one series per pod, no store label — as documented).
- **Visibility Write Request Rate per Store** went flat on **both** stores under an invalid `secondaryVisibilityWritingMode`, with no `visibility_persistence_*` error metric firing anywhere — the detection story in the playbook's section 1.2, now confirmed on a cluster rather than a unit probe.

### Re-tested on two separate Elasticsearch clusters
The field test above ran on **one** ES cluster with two indices (dual visibility "option B"), which cannot produce a connection-level failure of one store while the other stays healthy. Repeated on **two** ES clusters ("option A", two datastores) to test exactly that. Findings, all on the Visibility group:

- **A failing store never slows the *other* store.** The two writes are issued in parallel, so with the secondary hung and then stopped, **Visibility Write Latency per Store** held flat on the primary (p99 ~1s in all three states) while the secondary's read 49s. What breaks is the *task*, not the healthy store.
- **The healthy store absorbs the retry load.** Because the write only succeeds when both stores do, the task fails and retries, re-writing the store that already succeeded. **Visibility Write Request Rate per Store** rose several times over baseline on the **primary** while only the secondary was broken — load against a healthy store for records it had already stored on the first attempt.
- **Visibility Write Latency per Store / task processing latency reaches the full ack timeout per attempt** when a store is hung rather than gone: ~30s (`worker.ESProcessorAckTimeout`) per attempt, against under a second normally.
- **A store that is fully gone reports two `error_type` values at once** on **Visibility Errors by Type per Store** — `serviceerror_Unavailable` *and* `persistence_TimeoutError`. New connections fail immediately while documents already in the bulk buffer go on to hit the ack timeout. Seeing both together is one store gone, not two problems.
- **The store-blindness of the bulk processor panels is structural, and applies to all three.** Source confirms `NewProcessor` tags its metrics handler with the operation only; the `visibility_index_name` tag is applied around the visibility *manager*, not the store's bulk processor. So **ES Bulk Processor Errors by HTTP Status**, **ES Bulk Processor Queue Depth** and **ES Write Confirm Latency vs Ack Timeout** are all store-blind, not just the first. The readme now states this once for the whole group and on each of the three panels.
- **Recovery needs no manual replay.** With writes still retrying, restoring the store closed the gap on its own within seconds in both directions (hung→healthy and gone→healthy), with no duplicate records on the store that had stayed up.
