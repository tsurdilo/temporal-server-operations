...
|---|---|---|
| `temporal-server-alerts.yaml` | Grafana alerting provisioning file — drop into `provisioning/alerting/` and alerts load automatically on Grafana startup. Datasource referenced by name (`Prometheus`) so no UID replacement needed. | ✅ Essential Set complete (22 alerts) |
...
| 59c | Visibility Store Write Latency High | 16 — Visibility | Visibility Write Latency per Store | 2119 | `visibility_persistence_latency` p99 > 3s for a store — visibility slow; list/search operations degrading. |
...
| 78 | Shard Fleet Deficit | 8 — Shard Movement | Owned Shards (Total) | 2120 | `numshards_gauge < total` sustained 15m — a genuinely unowned shard; catches a stuck shard (e.g. Cassandra `range_id` divergence) after it stops churning and alert 34 goes quiet. |
| 79 | Shard Ownership Loss Persisting | 8 — Shard Movement | Persistence Errors Total by Operation | 77 | Sustained `ShardOwnershipLostError` on `UpdateShard` — an ownership loss that isn't recovering (queries the typed error metric, excluded from the generic error rate). Typically co-fires with 78. |
| 80 | History Task DLQ Stranding | 8 — Shard Movement | Dead-Lettered Tasks — Execution-Stranding | 2202 | Execution-stranding history tasks (activity retry/timeout, activity/WFT dispatch) being dead-lettered under a prolonged DB outage — not auto-retried, so executions strand until redriven. DB-agnostic. |
| 81 | Archival Backend Failing | 8a — Archival Health | Archival Attempt Error Rate | 2211 | Sustained archival backend errors (`archiver_archive_latency`, status=err) — the earliest archival-outage signal, ~1h before DLQ pressure begins. |
| 82 | History Task DLQ Write Failures | 8a — Archival Health | History Task DLQ Writes & Write Failures | 2213 | `task_dlq_failures` — DLQ writes themselves failing; the database can't even accept DLQ writes (active distress), often driven by a sustained archival outage (81). |

> **Dashboard UID for all panel links:** `temporal-overview-v1` (except 34j → `temporal-shard-io-v1`)
...
| 83 | Task Queue Partition Backlog Not Draining | ⚠️ Warning | Approximate Backlog Age by Partition (Task Queue Partitions dashboard) | Per-partition `approximate_backlog_age_seconds` above a workload-specific threshold, sustained for the `for` window — a partition's backlog isn't draining. Most often a `Write > Read` misconfiguration (a partition beyond the read count with no poller). **Not in the Essential Set:** the threshold is workload-specific and the metric is opt-in per queue (`metrics.breakdownByPartition`), so it can't be cluster-wide — scope it to instrumented queues and tune. Triage in the [Changing Task Queue Partitions playbook](../../../playbooks/change-task-queue-partitions.md#detecting-and-fixing-an-existing-write--read-misconfiguration). |
